Function Set-KeyProperty
{
    ...
    Process
    {
        ForEach ($Key In $Path)
        {
            ...
            {
                If ($Force.IsPresent) { $Key.Split(':')[1].TrimStart('\') | Grant-KeyAccess }
                Set-ItemProperty -LiteralPath $Key -Name $Name -Value $Value -Type $Type -Force -ErrorAction SilentlyContinue
            }
            Else
            {
                If ($Force.IsPresent -and (Test-Path -LiteralPath (Split-Path -LiteralPath $Key) -PathType Container)) { (Split-Path -LiteralPath $Key).Split(':')[1].TrimStart('\') | Grant-KeyAccess }
