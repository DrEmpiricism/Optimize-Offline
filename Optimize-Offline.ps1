...
<#
	...
	.PARAMETER ComponentCleanup
		Cleans-up the image Component Store. This does NOT perform an image base reset (ResetBase).
	...
	.EXAMPLE
		.\Optimize-Offline.ps1 -ImagePath "D:\Win Images\Win10Pro.iso" -Build 15063 -SelectApps -SystemApps -OptionalFeatures -OnDemandPackages -Harden -Local
	
	.EXAMPLE
		.\Optimize-Offline.ps1 -ISO "D:\Win Images\Win10Pro.iso" -Index 2 -Build 16299 -OnDemandPackages -Drivers "E:\DriverFolder" -ComponentCleanup -Local
...
(
	...
	[Parameter(HelpMessage = 'Cleans-up the image Component Store. This does NOT perform an image base reset (ResetBase).')]
...
Function Test-OfflineHives
{
	@("HKLM:\WIM_HKLM_SOFTWARE", "HKLM:\WIM_HKLM_SYSTEM",
		"HKLM:\WIM_HKCU", "HKLM:\WIM_HKU_DEFAULT") | % { If (Test-Path -Path $_) { $HivesLoaded = $true } }
	Return $HivesLoaded
}
...
If (!(Test-Admin))
{
	Write-Warning "Administrative access is required. Please re-launch $Script in an elevated shell."
	Break
}
...
If ($AllApps)
{
	Try
	{
		Get-AppxProvisionedPackage -Path $MountFolder -ScratchDirectory $TempFolder -LogPath $DISMLog | % {
...
If ($SystemApps)
{
	Try
	{
		...
		If (Test-Path -Path $InboxAppsKey)
		{
			...
			Start-Sleep 5
			Write-Output ''
			$InboxApps = (Get-ChildItem -Path $InboxAppsKey).Name.Split("\") | ? { $_ -like "*Microsoft.*" }
...
If ($SetRegistry -or $Harden)
{
	...
	If ($Build -ge '16273')
	{
		...
		@("HKLM:\WIM_HKLM_SOFTWARE\Classes\SystemFileAssociations\.3mf\Shell\3D Edit", "HKLM:\WIM_HKLM_SOFTWARE\Classes\SystemFileAssociations\.bmp\Shell\3D Edit",
			...
			"HKLM:\WIM_HKLM_SOFTWARE\Classes\SystemFileAssociations\.tiff\Shell\3D Edit") | % { Remove-Item -Path $_ -Recurse -Force }
	}
	...
	If ($Build -ge "15063")
	{
		...
		@("HKLM:\WIM_HKLM_SOFTWARE\Classes\SystemFileAssociations\.3ds\Shell\3D Print", "HKLM:\WIM_HKLM_SOFTWARE\Classes\SystemFileAssociations\.3mf\Shell\3D Print",
			...
			"HKLM:\WIM_HKLM_SOFTWARE\Classes\SystemFileAssociations\.stl\Shell\3D Print", "HKLM:\WIM_HKLM_SOFTWARE\Classes\SystemFileAssociations\.wrl\Shell\3D Print") | % { Remove-Item -Path $_ -Recurse -Force }
	}
	#****************************************************************
	Write-Output '' >> $WorkFolder\Registry-Optimizations.log
	...
	@("HKLM:\WIM_HKLM_SOFTWARE\Classes\AllFilesystemObjects\shellex\PropertySheetHandlers\{596AB062-B4D2-4215-9F74-E9109B0A8153}",
		...
		"HKLM:\WIM_HKLM_SOFTWARE\Classes\Drive\shellex\ContextMenuHandlers\{596AB062-B4D2-4215-9F74-E9109B0A8153}") | % { Remove-Item -Path $_ -Recurse -Force }
	#****************************************************************
	Write-Output '' >> $WorkFolder\Registry-Optimizations.log
	...
	Set-ItemProperty -Path "HKLM:\WIM_HKLM_SOFTWARE\Classes\Directory\Background\shell\runas\command" -Name "(default)" -Value "CMD /S /K PUSHD `"%V`"" -Type String
	...
	Set-ItemProperty -Path "HKLM:\WIM_HKLM_SOFTWARE\Classes\Directory\shell\runas\command" -Name "(default)" -Value "CMD /S /K PUSHD `"%V`"" -Type String
	#****************************************************************
	Write-Output '' >> $WorkFolder\Registry-Optimizations.log
	...
	Set-ItemProperty -Path "HKLM:\WIM_HKLM_SOFTWARE\Classes\Directory\Background\shell\ElevatedPowerShell\command" -Name "(default)" `
					 -Value "Powershell Start-Process PowerShell -Verb runas -ArgumentList '-NoExit', 'Push-Location -LiteralPath ''`"%V`"'''" -Type String
	...
	Set-ItemProperty -Path "HKLM:\WIM_HKLM_SOFTWARE\Classes\CABFolder\Shell\RunAs\Command" -Name "(default)" `
					 -Value "CMD /K DISM /ONLINE /ADD-PACKAGE /PACKAGEPATH:`"%1`"" -Type String
	#****************************************************************
	Write-Output '' >> $WorkFolder\Registry-Optimizations.log
...
If ($Harden)
{
	...
	#****************************************************************
	New-Container -Path "HKLM\WIM_HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\DeviceAccess\Global\{8BC668CF-7728-45BD-93F8-CF2B3B41D7AB}"
	New-Container -Path "HKLM\WIM_HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\DeviceAccess\Global\{992AFA70-6F47-4148-B3E9-3003349C1548}"
	New-Container -Path "HKLM\WIM_HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\DeviceAccess\Global\{21157C1F-2651-4CC1-90CA-1F28B02263F6}"
	New-Container -Path "HKLM\WIM_HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\DeviceAccess\Global\LooselyCoupled"
...
Try
{
	If ($AllApps -or $SystemAppsList -contains "Microsoft.XboxGameCallableUI" -or ((Get-AppxProvisionedPackage -Path $MountFolder | ? { $_.PackageName -like "*Xbox*" }).Count -lt 5))
	{
		Write-Output ''
...
{
	Clear-Host
	Write-Host "Acquiring Windows OnDemand Packages." -ForegroundColor Cyan
	Try
	{
		...
		$PackageName | % { $Append = $_.Replace("Package", "").Split("~")[0] }
		If ($RemoveOnDemand)
		{
			$PackageName | % {
...
{
	Clear-Host
	Write-Host "Acquiring Windows Optional Features." -ForegroundColor Cyan
	Try
	{
		...
		If ($RemoveFeatures)
		{
			$FeatureName | % {
				Write-Output ''
				Write-Log -Output "Disabling Optional Feature: $($_)" -LogPath $LogFile -Level Info
...
If ((Get-AppxProvisionedPackage -Path $MountFolder | ? { $_.PackageName -like "*Calculator*" }).Count.Equals(0))
{
	If ((Test-Path -Path "$PSScriptRoot\Win32Calc\win32calc.exe") -and (Test-Path -Path "$PSScriptRoot\Win32Calc\win32calc.exe.mui"))
	{
		Write-Output ''
		Write-Log -Output "Applying the Win32 Calculator." -LogPath $LogFile -Level Info
		Copy-Item -Path "$PSScriptRoot\Win32Calc\win32calc.exe" -Destination "$MountFolder\Windows\System32\win32calc.exe" -Force
		Copy-Item -Path "$PSScriptRoot\Win32Calc\win32calc.exe.mui" -Destination "$MountFolder\Windows\System32\en-US\win32calc.exe.mui" -Force
	}
	[void](Mount-OfflineHives)
	...
	[void](Dismount-OfflineHives)
	$CalcShell = New-Object -ComObject WScript.Shell
...
'@
	...
	{
		Out-File -FilePath $SetupScriptPath -InputObject $Defender, $Xbox, $SetupFinish -Encoding ASCII -Append
	}
	ElseIf ($DisableDefenderComplete -eq $true -and $DisableXboxComplete -ne $true)
	{
		Out-File -FilePath $SetupScriptPath -InputObject $Defender, $SetupFinish -Encoding ASCII -Append
	}
	ElseIf ($DisableDefenderComplete -ne $true -and $DisableXboxComplete -eq $true)
	{
		Out-File -FilePath $SetupScriptPath -InputObject $Xbox, $SetupFinish -Encoding ASCII -Append
	}
}
...
If ($ComponentCleanup)
{
	Try
	{
		If (!(Test-Path -Path "$MountFolder\Windows\WinSxS\pending.xml"))
		{
			Write-Output ''
			Write-Log -Output "Cleaning-up the Component Store." -LogPath $LogFile -Level Info
			Start-Sleep 3
			[void](Invoke-Expression -Command ("DISM /Image:$MountFolder /Cleanup-Image /StartComponentCleanup") -ErrorAction Stop)
		}
		Else
		{
			Break
	...
	{
		Write-Output ''
		Write-Log -Output "Cannot perform a Component clean-up while pending.xml is present in the image." -LogPath $LogFile -Level Warning
	}
}
