...
Function Set-Privacy
{
    ...
    Get-ScheduledTask -TaskName $Tasks -ErrorAction SilentlyContinue | Disable-ScheduledTask -ErrorAction SilentlyContinue

    # Gets the services from the array above, stops them and then disables them.
    Get-Service -Name $Services -ErrorAction SilentlyContinue | Stop-Service -Force -PassThru -WarningAction SilentlyContinue -ErrorAction SilentlyContinue | Set-Service -StartupType Disabled -ErrorAction SilentlyContinue

    # Disables Diagtrack autologging
    Set-AutologgerConfig -Name "AutoLogger-Diagtrack-Listener" -Start 0 -ErrorAction SilentlyContinue

    # Modifies the firewall rules and adds additional blocks for CompatTelRunner, DiagTrack and SmartScreen.
    Get-NetFirewallRule | Where-Object Group -Like "*@{*" | Remove-NetFirewallRule -ErrorAction SilentlyContinue
    Get-NetFirewallRule | Where-Object Group -EQ "DiagTrack" | Remove-NetFirewallRule -ErrorAction SilentlyContinue
    Get-NetFirewallRule | Where-Object DisplayGroup -EQ "Delivery Optimization" | Remove-NetFirewallRule -ErrorAction SilentlyContinue
    Get-NetFirewallRule | Where-Object DisplayGroup -Like "Windows Media Player Network Sharing Service*" | Remove-NetFirewallRule -ErrorAction SilentlyContinue
    New-NetFirewallRule -DisplayName "Block DiagTrack" -Action Block -Description "Block the DiagTrack Telemetry Service" -Direction Outbound -Name "Block DiagTrack" -Profile Any -Service DiagTrack -ErrorAction SilentlyContinue
    New-NetFirewallRule -DisplayName "Compatability Telemetry Runner Inbound" -Action Block -Description "Prevent CompatTelRunner Inbound Traffic." -Direction Inbound -Name "Compatability Telemetry Runner" -Profile Any -Program "%SystemDrive%\Windows\System32\CompatTelRunner.exe" -ErrorAction SilentlyContinue
    New-NetFirewallRule -DisplayName "Compatability Telemetry Runner Outbound" -Action Block -Description "Prevent CompatTelRunner Outbound Traffic." -Direction Outbound -Name "Compatability Telemetry Runner" -Profile Any -Program "%SystemDrive%\Windows\System32\CompatTelRunner.exe" -ErrorAction SilentlyContinue
    New-NetFirewallRule -DisplayName "SmartScreen Inbound" -Action Block -Description "Prevent SmartScreen Inbound Traffic." -Direction Inbound -Name "SmartScreen" -Profile Any -Program "%SystemDrive%\Windows\System32\smartscreen.exe" -ErrorAction SilentlyContinue
    New-NetFirewallRule -DisplayName "SmartScreen Outbound" -Action Block -Description "Prevent SmartScreen Outbound Traffic." -Direction Outbound -Name "SmartScreen" -Profile Any -Program "%SystemDrive%\Windows\System32\smartscreen.exe" -ErrorAction SilentlyContinue
    ...
    If (Test-Path -Path "$Env:SystemRoot\System32\CompatTelRunner.exe")
    {
        ...
        Remove-Item -Path "$Env:SystemRoot\System32\CompatTelRunner.exe" -Force -ErrorAction SilentlyContinue
    }
